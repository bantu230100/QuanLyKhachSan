...
{
    public partial class FormRoom : DevExpress.XtraEditors.XtraUserControl
    {
        RoomBUS phongBUS = new RoomBUS();
        public FormRoom()
        {
            InitializeComponent();
        ...
        List<RoomDTO> lstt = new List<RoomDTO>();
        ...
        private RoomDTO LayThongTinTimKiem()
        {
            RoomDTO thongtin = new RoomDTO();
        ...
        {
            RoomDTO thongtin = LayThongTinTimKiem();
            if (s == 1)
            {
                lstt = RoomBUS.DanhSachPhongYeuCau(thongtin);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
            ...
            {
                lstt = RoomBUS.DanhSachPhongTheoLoai(thongtin);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
            ...
            {
                lstt = RoomBUS.DanhSachPhongTheoLoaiTheoYeuCau(thongtin);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
            ...
            else if (s == 4)
            {
                ...
                lstt = RoomBUS.DanhSachPhongTheoDGTu(dgtu);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
            ...
            else if (s == 5)
            {
                ...
                lstt = RoomBUS.DanhSachPhongTheoKhoangDG(dgtu, dgden);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
            ...
            else if (s == 6)
            {
                ...
                lstt = RoomBUS.DanhSachPhongTheoDGDen(dgden);
                for (int i = 0; i < lstt.Count; i++)
                {
                    lstt[i].DonGia = Convert.ToDecimal(string.Format("{0:0,0}", lstt[i].DonGia));
                }
        ...
        {
            gcDanhSachPhong.DataSource = RoomBUS.DanhSachPhong();
            txtDonGiaTu.Text = "";
            txtDonGiaDen.Text = "";
        }
