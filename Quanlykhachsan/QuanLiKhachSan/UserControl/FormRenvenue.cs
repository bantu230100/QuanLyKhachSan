...
{
    public partial class FormRenvenue : DevExpress.XtraEditors.XtraUserControl
    {
        public FormRenvenue()
        {
            InitializeComponent();
        }
        List<SalesRepostDTO> lst = new List<SalesRepostDTO>();
        private void btnXem_Click(object sender, EventArgs e)
        {
            SalesRepostDTO infor = new SalesRepostDTO();
            List<SalesRepostDTO> dlst = new List<SalesRepostDTO>();
            if (txtNam.Text == "" && cmbThang.Text == "")
            {
                decimal sum = 0;
                lst = RepostBUS.TinhTongDoanhThuTheoLoaiPhong();
            ...
            else if (txtNam.Text != "" && cmbThang.Text != "")
            {
                ...
                lst = RepostBUS.DoanhThuTheoLoaiPhongTheoThang(infor);
                for (int i = 0; i < lst.Count; i++)
                {
                    lst[i].TongDoanhThu = Convert.ToDecimal(string.Format("{0:0,0}", lst[i].TongDoanhThu));
                }
